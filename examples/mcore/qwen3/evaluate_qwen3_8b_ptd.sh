...
# Positional overrides: 1=MASTER_PORT, 2=LR, 3=ASCEND_RT_VISIBLE_DEVICES, 4=OPTIMIZER
USER_MASTER_PORT=$1
USER_LR=$2
USER_VISIBLE=$3
USER_OPTIMIZER=$4
...
############################################
# 评测配置 (与 0.6B Muon 模板对齐)
############################################
LR=${USER_LR:-3e-4}
OPTIMIZER=${USER_OPTIMIZER:-muon}
EXP_NAME="qwen3_8b_4k_${OPTIMIZER}_lr${LR}"

# 路径配置
TOKENIZER_PATH="/sharedata/data/models/Qwen3-0.6B-Base"
CHECKPOINT="/sharedata/ckw/ckpt/${EXP_NAME}"
DATA_PATH="/sharedata/ckw/data/mmlu/data/test/"
TASK="mmlu"
