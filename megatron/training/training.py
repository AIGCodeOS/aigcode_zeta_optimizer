...
def train_step(forward_step_func, data_iterator,
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        if getattr(args, 'optimizer', None) in ['sophia', 'sophiamuon', 'zetamm', 'zetar', 'zetas', 'zetapro', 'adamuonsophia']:
            interval = getattr(args, 'sophia_hessian_interval', None)
            if interval is not None and interval > 0 and args.curr_iteration != 0 and args.curr_iteration % interval == 0:
                def _sophia_loss_func(loss_mask: torch.Tensor, logits: torch.Tensor):
                    args_ = get_args()
                    # Unify sampling precision with train_sophiag.py
                    # Ensure logits are float32 for stable sampling (mimics autocast behavior in SophiaG)
                    samp_dist = torch.distributions.Categorical(logits=logits.float())
                    sampled = samp_dist.sample().view(-1)
                    ...
                    losses = torch.nn.functional.cross_entropy(
                        ...
                        sampled,
                        reduction='none',
                    ).view(loss_mask.shape).float()
                    loss_mask_ = loss_mask.view(-1).float()
                    total_tokens = loss_mask_.sum()
                    loss_sum = torch.sum(losses.view(-1) * loss_mask_)
                    loss = torch.stack([loss_sum, total_tokens])
                    if args_.context_parallel_size > 1:
                    ...
                    # if mpu.is_pipeline_last_stage(ignore_virtual=True) and mpu.get_data_parallel_rank(with_context_parallel=True) == 0:
                    #     global_token_num = reporting_loss[1].item()
                    #     global_avg_loss = reporting_loss[0].item() / (global_token_num + 1e-12)
                    #     print(f"Sophia Sampled Loss: {global_avg_loss:.4f}", flush=True)

                    return loss[0].clone() / (loss[1].clone() + 1e-12), local_num_tokens

                def _sophia_forward_step(iterator, mdl):
                    args_ = get_args()
                    timers_ = get_timers()
                    timers_('batch-generator', log_level=2).start()
                    batch_ = get_batch_on_this_tp_rank(iterator)
                    batch_ = get_batch_on_this_cp_rank(batch_)
                    tokens, labels, loss_mask, attention_mask, position_ids = batch_.values()
                    timers_('batch-generator').stop()
                    if args_.use_legacy_models:
                        output_tensor_ = mdl(tokens, position_ids, attention_mask)
                    else:
                        output_tensor_ = mdl(tokens, position_ids, attention_mask, labels=None, loss_mask=None)
                    return output_tensor_, functools.partial(_sophia_loss_func, loss_mask)

                forward_backward_func = get_forward_backward_func()
                _ = forward_backward_func(
                    forward_step_func=_sophia_forward_step,
                ...
                # Prepare gradients (reduce, unscale, copy to main) for Hessian update
                found_inf = False
                if hasattr(optimizer, 'prepare_grads'):
                    found_inf = optimizer.prepare_grads()
                
                if not found_inf:
                    if hasattr(optimizer, 'chained_optimizers'):
                        for optim_instance in optimizer.chained_optimizers:
                            base_opt = getattr(optim_instance, 'optimizer', None)
                            if base_opt is not None and hasattr(base_opt, 'update_hessian'):
                                base_opt.update_hessian()
                    else:
                        base_opt = getattr(optimizer, 'optimizer', None)
                        if base_opt is not None and hasattr(base_opt, 'update_hessian'):
                            base_opt.update_hessian()
    ...
    if getattr(args, 'optimizer', None) in ['sophia', 'sophiamuon', 'zetamm', 'zetar', 'zetas', 'zetapro', 'adamuonsophia', 'zeta', 'adamuon']:
        current_bs = get_current_global_batch_size() * args.seq_length
        def _update_bs(opt, val):
            if hasattr(opt, 'param_groups'):
                for group in opt.param_groups:
                    group['bs'] = val
            if hasattr(opt, 'optimizer') and opt.optimizer is not opt:
                _update_bs(opt.optimizer, val)
            if hasattr(opt, 'chained_optimizers'):
                for chained in opt.chained_optimizers:
                    _update_bs(chained, val)
        _update_bs(optimizer, current_bs)

        hessian_norm_l1 = torch.tensor(0.0, device=torch.cuda.current_device())
        hessian_norm_l2_sq = torch.tensor(0.0, device=torch.cuda.current_device())

        def _get_hessian_norms(opt):
            nonlocal hessian_norm_l1, hessian_norm_l2_sq
            if hasattr(opt, 'param_groups'):
                for group in opt.param_groups:
                    for p in group['params']:
                        if hasattr(opt, 'state') and p in opt.state and 'hessian' in opt.state[p]:
                            hessian = opt.state[p]['hessian']
                            if tensor_parallel.param_is_not_tensor_parallel_duplicate(p):
                                hessian_norm_l1 += hessian.detach().norm(1)
                                hessian_norm_l2_sq += hessian.detach().norm(2) ** 2

            if hasattr(opt, 'optimizer') and opt.optimizer is not opt:
                _get_hessian_norms(opt.optimizer)
            if hasattr(opt, 'chained_optimizers'):
                for chained in opt.chained_optimizers:
                    _get_hessian_norms(chained)

        _get_hessian_norms(optimizer)

        clipping_ratio = 0.0
        def _get_clipping_ratio(opt):
            nonlocal clipping_ratio
            if hasattr(opt, 'clipping_ratio'):
                clipping_ratio = opt.clipping_ratio
            if hasattr(opt, 'optimizer') and opt.optimizer is not opt:
                _get_clipping_ratio(opt.optimizer)
            if hasattr(opt, 'chained_optimizers'):
                for chained in opt.chained_optimizers:
                    _get_clipping_ratio(chained)
        
        _get_clipping_ratio(optimizer)

        optimizer_stats = {}
        def _get_optimizer_stats(opt):
            nonlocal optimizer_stats
            if hasattr(opt, 'stats') and isinstance(opt.stats, dict):
                optimizer_stats.update(opt.stats)
            if hasattr(opt, 'optimizer') and opt.optimizer is not opt:
                _get_optimizer_stats(opt.optimizer)
            if hasattr(opt, 'chained_optimizers'):
                for chained in opt.chained_optimizers:
                    _get_optimizer_stats(chained)
        
        _get_optimizer_stats(optimizer)

        # Reduce L1 norm
        torch.distributed.all_reduce(hessian_norm_l1, group=mpu.get_tensor_model_parallel_group())
        torch.distributed.all_reduce(hessian_norm_l1, group=mpu.get_pipeline_model_parallel_group())
        if args.use_distributed_optimizer:
            torch.distributed.all_reduce(hessian_norm_l1, group=mpu.get_data_parallel_group())

        # Reduce L2 norm squared
        torch.distributed.all_reduce(hessian_norm_l2_sq, group=mpu.get_tensor_model_parallel_group())
        torch.distributed.all_reduce(hessian_norm_l2_sq, group=mpu.get_pipeline_model_parallel_group())
        if args.use_distributed_optimizer:
            torch.distributed.all_reduce(hessian_norm_l2_sq, group=mpu.get_data_parallel_group())

        hessian_norm_l2 = hessian_norm_l2_sq.item() ** 0.5
        
        stats_str = ""
        if optimizer_stats:
            stats_str = ", " + ", ".join([f"{k}: {v:.6f}" if isinstance(v, (float, int)) else f"{k}: {v}" for k, v in optimizer_stats.items()])

        print_rank_0(f'hessian_norm_l1: {hessian_norm_l1.item()}, hessian_norm_l2: {hessian_norm_l2}, clipping_ratio: {clipping_ratio}{stats_str}')
