...
def parse_args(extra_args_provider=None, ignore_unknown_args=False):
    ...
    parser = argparse.ArgumentParser(description='Megatron-LM Arguments',
                                     allow_abbrev=False)
    print("adding megatron arguments...")
    parser = add_megatron_arguments(parser)
    print("added megatron arguments.")  
    # Custom arguments.
    if extra_args_provider is not None:
        parser = extra_args_provider(parser)
    print("parsing args... ignore_unknown_args=", ignore_unknown_args)    
    # Parse.
    ...
    print("setting rank and world size from env...")
    # Args from environment
    args.rank = int(os.getenv('RANK', '0'))
    args.world_size = int(os.getenv("WORLD_SIZE", '1'))
    print(f"set rank: {args.rank}, world size: {args.world_size}")
    return args
...
def validate_args(args, defaults={}):
    print("> validating megatron arguments ...")
    # Temporary
...
def _add_logging_args(parser):
    ...
    group.add_argument('--wandb-entity', type=str, default='',
                       help='The wandb experiment entity.')
    group.add_argument('--wandb-group', type=str, default='',
                       help='The wandb experiment group.')    
...
def _add_regularization_args(parser):
    ...
    group.add_argument('--zetamm-ns-steps', type=int, default=5,
                       help='Newton-Schulz iteration steps for Zeta-MM optimizer')
    group.add_argument('--zetamm-trust-region-threshold', type=float, default=0.1,
                       help='Trust region threshold for Zeta-MM optimizer')
    group.add_argument('--zetamm-momentum', type=float, default=0.95,
                       help='Momentum factor for Zeta-MM optimizer')
    group.add_argument('--zetamm-nesterov', action='store_true', default=True,
                       help='Whether to use Nesterov momentum for Zeta-MM optimizer')
    group.add_argument('--muon-momentum', type=float, default=0.95,
                       help='Momentum factor for Muon optimizer')
    group.add_argument('--muon-nesterov', action='store_true', default=True,
                       help='Whether to use Nesterov momentum for Muon optimizer')
    group.add_argument('--muon-ns-steps', type=int, default=5,
                       help='Newton-Schulz iteration steps for Muon optimizer')
    group.add_argument('--zeta-momentum', type=float, default=0.95,
                       help='Momentum factor for Zeta optimizer')
    group.add_argument('--zeta-nesterov', action='store_true', default=True,
                       help='Whether to use Nesterov momentum for Zeta optimizer')
    group.add_argument('--zeta-ns-steps', type=int, default=4,
                       help='Newton-Schulz iteration steps for Zeta optimizer')
    group.add_argument('--zeta-weight-decay', type=float, default=0.01,
                       help='Weight decay coefficient for Zeta optimizer')
    group.add_argument('--zeta-beta1', type=float, default=0.95,
                       help='First coefficient for Zeta optimizer')
    group.add_argument('--zeta-beta2', type=float, default=0.99,
                       help='Second coefficient for Zeta optimizer')
    group.add_argument('--zeta-rho', type=float, default=0.04,
                       help='Rho parameter for Zeta optimizer')
    group.add_argument('--spmc-beta1', type=float, default=0.95,
                       help='First coefficient for SPMC optimizer')
    group.add_argument('--spmc-beta2', type=float, default=0.99,
                       help='Second coefficient for SPMC optimizer')
    group.add_argument('--spmc-rho', type=float, default=0.04,
                       help='Rho parameter for SPMC optimizer')
    group.add_argument('--spmc-ns-steps', type=int, default=5,
                       help='Newton-Schulz iteration steps for SPMC optimizer')
    group.add_argument('--spmc-eps', type=float, default=1e-15,
                       help='Epsilon parameter for SPMC optimizer')
    group.add_argument('--spmc-use-cayley', action='store_true', default=True,
                       help='Whether to use Cayley correction for SPMC optimizer')
    group.add_argument('--vpmc-beta1', type=float, default=0.95,
                       help='First coefficient for VPMC optimizer')
    group.add_argument('--vpmc-beta2', type=float, default=0.99,
                       help='Second coefficient for VPMC optimizer')
    group.add_argument('--vpmc-rho', type=float, default=0.04,
                       help='Rho parameter for VPMC optimizer')
    group.add_argument('--vpmc-ns-steps', type=int, default=5,
                       help='Newton-Schulz iteration steps for VPMC optimizer')
    group.add_argument('--vpmc-eps', type=float, default=1e-12,
                       help='Epsilon parameter for VPMC optimizer')
    group.add_argument('--vpmc-use-cayley', action='store_true', default=True,
                       help='Whether to use Cayley correction for VPMC optimizer')
    group.add_argument('--zeas-momentum', type=float, default=0.95,
                       help='Momentum factor for Zetas optimizer')
    group.add_argument('--zeas-nesterov', action='store_true', default=True,
                       help='Whether to use Nesterov momentum for Zetas optimizer')
    group.add_argument('--zeas-ns-steps', type=int, default=5,
                       help='Newton-Schulz iteration steps for Zetas optimizer')
    group.add_argument('--zeas-weight-decay', type=float, default=0.01,
                       help='Weight decay coefficient for Zetas optimizer')
    group.add_argument('--zeas-trust-region-threshold', type=float, default=0.1,
                       help='Trust region threshold for Zetas optimizer')
    group.add_argument('--zeas-beta1', type=float, default=0.965,
                       help='First coefficient for Zetas optimizer')
    group.add_argument('--zeas-beta2', type=float, default=0.99,
                       help='Second coefficient for Zetas optimizer')
    group.add_argument('--zeas-rho', type=float, default=0.04,
                       help='Rho parameter for Zetas optimizer')
    group.add_argument('--zeas-mode', type=str, default='post_scale',
                       help='Mode for Zetas optimizer')
    group.add_argument('--dpmc-beta1', type=float, default=0.95,
                       help='First coefficient for DPMC optimizer')
    group.add_argument('--dpmc-beta2', type=float, default=0.99,
                       help='Second coefficient for DPMC optimizer')
    group.add_argument('--dpmc-rho', type=float, default=0.04,
                       help='Rho parameter for DPMC optimizer')
    group.add_argument('--dpmc-ns-steps', type=int, default=5,
                       help='Newton-Schulz iteration steps for DPMC optimizer')
    group.add_argument('--dpmc-eps', type=float, default=1e-10,
                       help='Epsilon parameter for DPMC optimizer')
    group.add_argument('--dpmc-use-cayley', action='store_true', default=True,
                       help='Whether to use Cayley correction for DPMC optimizer')
    return parser
...
def _add_training_args(parser):
    ...
    group.add_argument('--optimizer', type=str, default='adam',
                       choices=['adam', 'sgd', 'zetamm', 'muon', 'zeta', 'spmc', 'vpmc', 'zetas', 'zetar','dpmc', 'zetapro', 'sophiamuon','kfac', 'adamuon', 'muonmagma', 'zetasoliton', 'mixmuon', 'sophia', 'zetahspro', 'adamuonadp'],
...
def _add_data_args(parser):
    ...
    group.add_argument('--data-dir', type=str, default=None,
                       help='Directory to recursively search for .bin and .idx files to add to the dataset. '
                       'All found files will be added to the data-path list.')
