...
class Timers:
    ...
    def _get_global_min_max_time(self, names, reset, barrier, normalizer):
        ...
        if rank_name_to_time is None:
            return {}
        rank_name_to_time = rank_name_to_time.permute(1, 0).cpu().tolist()
        name_to_min_max_time = {}
        for i, name in enumerate(names):
            rank_to_time = [x for x in rank_name_to_time[i] if x > 0.0]
            if rank_to_time:
    ...
    def _get_all_ranks_time_string(self, names, reset, barrier, normalizer):
        ...
        for i, name in enumerate(names):
            ...
            for rank in range(torch.distributed.get_world_size()):
                val = float(rank_name_to_time[rank, i].item())
                if val > 0.0:
                    no_reported_timing = False
                    if not_yet_found:
                        not_yet_found = False
                        output_string += '\n  {}:'.format(name)
                    output_string += '\n     rank {:2d}: {:.2f}'.format(rank, val / normalizer)
