...
class SophiaMuon(optim.Optimizer):
    def __init__(self, params, lr=1e-4, betas=(0.965, 0.99), rho=4,
    ...
    def step(self, closure=None, bs=5120):
        ...
        for group in self.param_groups:
            ...
            current_bs = 1048576  #group.get('bs', bs)
            ...
            clamped, numel = sophiamuon(params_with_grad,
                    grads,
                    exp_avgs,
                    hessian,
                    state_steps,
        ...
        if total_numel > 0:
            self.clipping_ratio = total_clamped / total_numel
        else:
            self.clipping_ratio = 0.0
...
def sophiamuon(params: List[torch.Tensor],
    ...
    return _single_tensor_sophiamuon(params,
                           grads,
                           exp_avgs,
                           hessian,
                           state_steps,
...
def _single_tensor_sophiamuon(params: List[torch.Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if param.ndim >= 2:
            ...
            a, b = param.shape[:2]
            scale = 0.2 * math.sqrt(a * b) / ortho_direction.sign().norm()
            ...
            if capturable:
                ...
                #norm = (ortho_direction / denom).norm()
                update_val = ortho_direction.sign() * scale
                ...
                # Statistic
                total_clamped += (raw_val.abs() > 1.0).sum().item()
                total_numel += raw_val.numel()
            ...
            # Statistic
            raw_val = exp_avg.abs().div(denom)
            total_clamped += (raw_val > 1.0).sum().item()
            total_numel += raw_val.numel()

            ratio = raw_val.clamp(None, 1)
    ...
    return total_clamped, total_numel
