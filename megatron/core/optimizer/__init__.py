...
from .zetasoliton import ZetaMuonSoliton
from .muon import Muon
from .adamuon import AdaMuon
from .adamuon_adp import AdaMuonParamType as AdaMuonAdp
from .mixmuon import MixMuon
from .sophiamuon import SophiaMuon
from .sophia import SophiaG
from .zeta_hspro import ZetaHSPro
...
def _get_megatron_optimizer_based_on_param_groups(
    ...
    if param_groups:
        if config.optimizer_cpu_offload:
            ...
            elif config.optimizer == 'zeta':
                # Zeta doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'muonmagma':
                # MuonMagma doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'muon':
                # Muon doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'zetasoliton':
                # ZetaSoliton doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'adamuon':
                # AdaMuon doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'adamuonadp':
                # AdaMuonAdp doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'mixmuon':
                # MixMuon doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'sophiamuon':
                # SophiaMuon doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'sophia':
                # Sophia doesn't support CPU offloading yet, fall back to Adam
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            elif config.optimizer == 'zetahspro':
                # ZetaHSPro doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
            else:
            ...
            if config.optimizer == 'adam':
                ...
                # Zeta doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # ZetaSoliton doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # MuonMagma doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # Muon doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # AdaMuon doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # AdaMuonAdp doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # MixMuon doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # SophiaMuon doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
                ...
                # Sophia doesn't support CPU offloading yet, use Adam as fallback
                gpu_optimizer_cls = Adam
                cpu_optimizer_cls = CPUAdam
                optimizer_defaults = dict(
                    lr=config.lr,
            ...
            optimizer = Zeta(
                param_groups,
                lr=config.lr,
                ns_steps=config.zeta_ns_steps,
                weight_decay=config.zeta_weight_decay,
                betas=(config.zeta_beta1, config.zeta_beta2),
            )
            init_state_fn = None
        elif config.optimizer == 'muonmagma':
            optimizer = MuonMagma(
            ...
            optimizer = ZetaMuonSoliton(
                param_groups,
                lr=config.lr,
                weight_decay=config.weight_decay,
                betas=(config.adam_beta1, config.adam_beta2),
                momentum=config.zeta_momentum,
            )
            init_state_fn = None
        elif config.optimizer == 'adamuon':
            optimizer = AdaMuon(
