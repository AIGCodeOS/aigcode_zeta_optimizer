...
class Mg2HfConvert(Convert):
    ...
    def _valid_parameter(self):
        ...
        if getattr(self.load_model, 'num_experts', None) and self.expert_tensor_parallel_size is not None:
    ...
    def set_model_preprocess(self, hf_weight, mg_weight):
        ...
        if not self.load_model.untie_embeddings_and_output_weights:
            GLOBAL_LM_HEAD_WEIGHTS = emb_weights.clone()
    ...
    def set_model_layer_attn(self, hf_weight, mg_weight, hf_layer_idx, local_layer_idx, mtp_layer_flag=False):
        ...
        if getattr(self.load_model, "add_qkv_bias", False) or getattr(self.load_model, "enable_dsa_indexer", False):
        ...
        if self.load_model.qkv_type == "pack_mla":
            ...
            if getattr(self.load_model, "add_qkv_bias", False):
    ...
    def set_model_layer_mlp(self, hf_weight, mg_weight, hf_layer_idx, local_layer_idx, mtp_layer_flag=False):
        ...
        if hf_layer_idx >= self.first_k_dense_replace:
            ...
            if getattr(self.load_model, "router_bias", False):
