...
class ZetaMuonSoliton(optim.Optimizer):
    ...
    性能目标：~5秒/step（与 Muon 相当）
    
    Args:
        params: 模型参数
        lr: 学习率 (default: 3e-4)
        betas: (beta1, beta2) for momentum (default: (0.9, 0.95))
        eps: 数值稳定性 (default: 1e-8)
        weight_decay: 权重衰减 (default: 0.01)
        momentum: 动量系数 (default: 0.95)
        ns_steps: Newton-Schulz 步数 (default: 5)
        hessian_update_freq: Hessian 更新频率 (default: 10)
        gamma: Hessian 更新率 (default: 0.01)
        
        # 孤子参数
        use_soliton: 是否使用孤子检测 (default: True)
        num_solitons: 孤子数量 (default: 4)
        soliton_ns_steps: 孤子检测的 NS 步数 (default: 3)
        coherence_threshold: 相干性阈值 (default: 0.1)
        rg_projection_strength: RG 投影强度 (default: 0.3)
        
        # 轻量级特性（可选）
        use_light_torsion: 是否使用轻量级挠率 (default: False)
        torsion_strength: 挠率强度 (default: 0.05)
        use_short_memory: 是否使用短期记忆 (default: False)
        memory_length: 记忆长度 (default: 5)
...
bad：        lr: float = 5e-3,
        ...
        num_solitons: int = 8,  # 只用 1 个孤子
        soliton_ns_steps: int = 5,  # 只用 1 步迭代
        coherence_threshold: float = 0.1,
        rg_projection_strength: float = 0.5,
        # 轻量级特性
        use_light_torsion: bool = True,
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if p.ndim >= 2:
                    ...
                    # 轻量级挠率（可选）
                    # ========================================
                    torsion_correction = torch.zeros_like(base_direction)
                    
                    if use_light_torsion and state["step"] > 1:
                        M = exp_avg
                        G = base_direction
                        
                        # 迹近似
                        trace_MG = (M * G).sum()
                        torsion_scale = torch.tanh(trace_MG / (M.norm() * G.norm() + eps))
                        torsion_correction = torsion_strength * torsion_scale * G
                    
                    # ========================================
                    # 短期记忆（可选）
                    # ========================================
                    memory_correction = torch.zeros_like(base_direction)
                    
                    if use_short_memory and len(state["grad_history"]) >= 3:
                        history = list(state["grad_history"])
                        weights = [0.5 ** i for i in range(len(history))]
                        total_weight = sum(weights)
                        
                        memory_grad = torch.zeros_like(base_direction)
                        for w, g in zip(weights, history):
                            memory_grad += (w / total_weight) * g
                        
                        memory_norm = memory_grad.norm() + eps
                        memory_grad = memory_grad / memory_norm * base_scale
                        
                        memory_correction = 0.1 * memory_grad
